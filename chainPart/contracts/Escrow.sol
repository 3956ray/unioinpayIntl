...
contract Escrow is ReentrancyGuard {
    ...
    function charge(
        ...
        // 将代币转给接收者和费用接收者
        _distributeTokens(paymentInfo.operator, paymentInfo.token, paymentInfo.receiver, amount, feeBps, feeReceiver);
    }
    ...
    function _distributeTokens(
        ...
        // 从TokenStore转移代币给接收者
        if (receiverAmount > 0) {
            _sendTokens(operator, token, receiver, receiverAmount);
        }

        // 从TokenStore转移费用给费用接收者
        if (feeAmount > 0) {
            _sendTokens(operator, token, feeReceiver, feeAmount);
        }
    }
}